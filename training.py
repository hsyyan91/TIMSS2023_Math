...
# # all_results [SEED, target, model_name, split, rmse, mae, r2
# results_df = pd.DataFrame(all_results, columns=["Seed", "Target", "Model", "Split", "RMSE", "MAE", "R2"])

# # Compare by test set
# test_results = results_df[results_df["Split"] == "Test"].copy()

# # 将每一个 (Seed, Target) 组合看作一个独立的“评估环境/数据集”
# # 使用 pivot 将模型名称变为列，每一行代表在某个种子和某个PV下的表现
# # 以 RMSE 为例（如果想用 R2，把 values="RMSE" 改为 "R2"）
# pivot_df = test_results.pivot_table(index=["Seed", "Target"], columns="Model", values="RMSE")

# print("整理后的数据矩阵（部分）：")
# print(pivot_df.head())

# # Extract the performance vectors for the four models
...
# knn_scores = pivot_df["KNN"].dropna().values

# # Friedman test
# stat, p_value = stats.friedmanchisquare(rf_scores, xgb_scores, lgbm_scores, knn_scores)
...
# if p_value < 0.05:
#     print("There are significant differences in the predictive performance of these three models.")
# else:
#     print("There are no significant differences in the predictive performance of these three models.")
